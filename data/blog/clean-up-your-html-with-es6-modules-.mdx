...
Another _tip_: since we are talking about the named `export` we can also cast names, till now if we will `import` `doAnimationForMe` we have to use the same name `doAnimationForMe` or will cause an error. for rename exported part:
