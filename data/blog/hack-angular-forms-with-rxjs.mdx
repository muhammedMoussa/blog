...
- since angular coming with Rxjs, so the tip is **_Observables_** not Reactive or TD Forms, yeah we will discuss how to manage the forms by only observables and data binding nothing more.
...
```html
<button
  [class]="user.age && user.age < 18 ? 'cursor-not-allowed': 'cursor-pointer'"
  (click)="onSubmit()"
>
  Save
</button>
